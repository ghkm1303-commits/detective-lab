...
const ParasiteGame = ({ organisms, selectedClass, gameMode, onGameEnd, onBack, currentLang }) => {
  ...
  useEffect(() => {
    let pool = organisms.filter(o => o.category !== 'entomology' && o.category !== 'overview');
    if (selectedClass) {
      pool = pool.filter(o => o.category === selectedClass);
    }

    if (pool.length > 0) {
      const organism = pickRandomOrganism(pool);
